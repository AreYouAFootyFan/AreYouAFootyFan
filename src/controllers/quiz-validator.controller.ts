...
export class QuizValidatorController {
  ...
  static async validateQuiz(request: Request, response: Response, next: NextFunction): Promise<void> {
    try {
  ...
  static async validateQuestion(request: Request, response: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const validationResult = await QuizValidatorService.validateQuestion(questionId);
      response.json(validationResult);
    } catch (error) {
      next(error);
    }
