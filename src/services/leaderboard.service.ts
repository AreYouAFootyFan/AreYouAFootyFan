...
export class LeaderboardService {
   ...
   */
  static async getUserRank(userId: number): Promise<{ rank: number; total_points: number }> {
    try {
      const result = await db.query('SELECT * FROM get_user_rank($1)', [userId]);
      
      if (result.rows.length === 0) {
        return { rank: 0, total_points: 0 };
      }
      
      const points = await db.query('SELECT * FROM get_total_points($1)', [userId]);
