...
export class QuizValidatorService {
  ...
  static async validateQuestion(questionId: number): Promise<QuestionValidationResult> {
    ...
    if (answers.length < 4) {
      validationMessages.push(`Question needs ${4 - answers.length} more answer(s)`);
    } else if (answers.length > 4) {
      validationMessages.push(`Question has ${answers.length - 4} too many answers`);
    }
    
    if (correctAnswers.length === 0) {
      validationMessages.push('Question needs 1 correct answer');
    } else if (correctAnswers.length > 1) {
      validationMessages.push(`Question has ${correctAnswers.length - 1} too many correct answers`);
    }
