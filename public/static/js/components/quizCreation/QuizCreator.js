...
class QuizCreator extends HTMLElement {
  constructor() {
    ...
    this.currentQuestionId =
      localStorage.getItem("current_question_id") || null;
  ...
  createSidebar() {
    ...
    step1.className = `step-indicator ${
      this.viewMode === "quiz" ? "active" : this.isEditing ? "completed" : ""
    }`;
    ...
    step2.className = `step-indicator ${
      this.viewMode === "questions" ? "active" : ""
    }`;
    ...
    step3.className = `step-indicator ${
      this.viewMode === "question-form" || this.viewMode === "answers"
        ? "active"
        : ""
    }`;
  ...
  createQuizFormView() {
    ...
    quizFormContainer.className = `creator-view ${
      this.viewMode === "quiz" ? "" : "hidden"
    }`;
  ...
  createQuestionsView() {
    ...
    questionsContainer.className = `creator-view ${
      this.viewMode === "questions" ? "" : "hidden"
    }`;
    ...
    infoText.textContent =
      "Add at least 5 questions with 4 answer options each (1 correct) to make the quiz playable.";
    ...
    questionsList.addEventListener("add-question", () =>
      this.showQuestionForm()
    );
    questionsList.addEventListener("edit-question", (e) =>
  ...
  createQuestionFormView() {
    ...
    questionFormContainer.className = `creator-view ${
      this.viewMode === "question-form" ? "" : "hidden"
    }`;
    ...
    const questionId =
      this.currentQuestionId || localStorage.getItem("current_question_id");
  ...
  createAnswersView() {
    ...
    answersContainer.className = `creator-view ${
      this.viewMode === "answers" ? "" : "hidden"
    }`;
    ...
    answersList.addEventListener("back-to-questions", () =>
  ...
  async loadInitialData() {
    try {
      ...
      await Promise.all([this.loadCategories(), this.loadDifficulties()]);
  ...
  async loadDifficulties() {
    try {
      ...
      this.showNotification(
        "Failed to load difficulty levels. Please try again.",
        "error"
      );
    }
  }
  ...
  async loadQuizData() {
    ...
    try {
      ...
      const quizTitleElement = this.shadowRoot.querySelector(
        "#current-quiz-title"
      );
      if (quizTitleElement) {
  ...
  async loadQuestions() {
    ...
    try {
      ...
      const quizValidation = await window.quizValidatorService.validateQuiz(
        this.quizId
      );
  ...
  async handleQuizSubmit(event) {
    ...
    try {
      ...
      if (isNew) {
        ...
        this.showNotification(
          "Quiz created successfully! Now you can add questions."
        );
      } else {
      ...
      this.showNotification(
        "Failed to save quiz: " + (error.message || "Unknown error"),
        "error"
      );
    }
  }
  ...
  async handleQuestionSubmit(event) {
    ...
    try {
      ...
      this.showNotification(
        "Failed to save question: " + (error.message || "Unknown error"),
        "error"
      );
    }
  }
  ...
  async handleEditQuestion(questionId) {
    try {
      ...
      this.showNotification(
        "Failed to load question: " + (error.message || "Unknown error"),
        "error"
      );
    }
  }
