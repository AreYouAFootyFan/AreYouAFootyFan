...
class QuestionsList extends HTMLElement {
  ...
  createQuestionCard(question, index) {
    ...
    const statusText =
      question.validation_messages && question.validation_messages.length > 0
    ...
    deleteButton.className = `action-btn delete-question ${
      this.questions.length > 5 ? "" : "hidden"
    }`;
    ...
    deleteButton.addEventListener("click", () => {
      this.dispatchEvent(
        new CustomEvent("delete-question", {
          detail: { questionId: question.question_id },
          bubbles: true,
          composed: true,
        })
    ...
    manageAnswersBtn.addEventListener("click", () => {
      this.dispatchEvent(
        new CustomEvent("manage-answers", {
          detail: {
